<?php 
	...
	while (!feof($arquivo)) {
		...
		$etiqueta = $dados[0];
		$buscatoner = "SELECT * FROM Itoner WHERE etiqueta = '$etiqueta' ";
    	$encontrou = $banco->query($buscatoner);
    	if(mysqli_num_rows($encontrou) < 1){
    		$ativo = 1;
    		$sqlInsert = "INSERT INTO Itoner (etiqueta,modelo,ativo) VALUES ('$etiqueta','$dados[2]','$ativo')";
		...
		if ($dados[16] =='') {		
			if ( !empty($linha)){				
			$sqlInsert = "INSERT INTO Importa (etiqueta,recarga,modelo,pesoi,pesof, po, lixo, tempo, cil, rmag, pcr, ldos, llimp, chip, observacao, tecnico ) VALUES ('$dados[0]','$recarga','$dados[2]','$dados[3]','$dados[4]','$dados[5]','$dados[6]','$dados[7]', '$cil', '$rmag', '$pcr', '$ldos', '$llimp', '$chip', '$dados[14]', '$tecnico' )";
        	$result = $banco->query($sqlInsert);}
		} else {
