<?php
       ...
       // print_r('Senha: ' . $senha);
    	...
    	if(mysqli_num_rows($encontrou) < 1){
    		echo "<script> alert('Erro...: Etiqueta não Cadastrada!');</script>";
    		echo "<script> location.href='destdata.php';</script>";
    	...
    	$buscahistorico = "SELECT *, TIME_FORMAT(tempo, '%H:%i') as tempo, DATE_FORMAT(recarga,'%d-%m-%y') as recarga ,DATE_FORMAT(saida,'%d-%m-%y') as saida FROM Historico WHERE etiqueta = '$etiqueta' ORDER BY codigo DESC ";
	...
	<div class="cabecalho" id="dvcab">
		<div class="procura">
			<form action="novarecarga.php" method="POST">
				...
				<div class="executa">
					<div class="centro">
						<button class="botao2" name="voltar" value="Voltar" ><a href="destina.php"> Voltar</a> </button>
					</div>
 		   		</div>
 		   	</form>
 		</div>
